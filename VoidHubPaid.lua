...
local nickAtual = LP.Name:lower()

for _, nickPermitido in ipairs(Whitelist) do
    if nickPermitido ~= "" and nickPermitido:lower() == nickAtual then
        isWhitelisted = true
        break
    end
end
...
if not isWhitelisted then
    LP:Kick("\n❌ ACESSO NEGADO - VOID HUB ❌\nSeu nick (" .. LP.Name .. ") não está na Whitelist.\nCompre uma Key Perm para acessar.")
    return 
end
-- ===================== FIM DA WHITELIST =====================
...
local State = {
        ...
        antiRagdoll1Enabled = false, antiRagdoll2Enabled = false,
        ...
        autoTpDownEnabled = false, autoTpDownY = 20, autoTpDownDelay = 0.5,
        toolSpammerEnabled = false,
        ...
        keyLock = Enum.KeyCode.Unknown,
        keyToolSpam = Enum.KeyCode.Unknown,
        keyAutoGrab = Enum.KeyCode.Unknown,
        keyInfJump = Enum.KeyCode.Unknown,
        keyAutoTpDownToggle = Enum.KeyCode.Unknown,
        keyAntiRag1 = Enum.KeyCode.Unknown,
        keyAntiRag2 = Enum.KeyCode.Unknown,
        keyFps = Enum.KeyCode.Unknown,
        keyMedusa = Enum.KeyCode.Unknown,
        keyUnwalk = Enum.KeyCode.Unknown,
        keyCarry = Enum.KeyCode.Unknown,
}

local UpdateKeybindUI

-- Forward declarations para funções de movimento e toggle (evitando erros de variável global)
local startAutoLeft, stopAutoLeft
local startAutoRight, stopAutoRight
local startAntiRagdoll1, stopAntiRagdoll1
...
local setAutoLeft, setAutoRight, setCarryToggle
local setInstaGrab, setTargetLock, setToolSpammer, setInfJump, setAntiRag1, setAntiRag2, setFps, setMedusaCounter, setAutoTpDown
local setAnimToggle, setUnwalkToggle

local normalBox, carryBox
local setSpeedToggleUI
...
local function autoSaveConfig()
        ...
        task.delay(0.5, function()
                local cfg = {
                        ...
                        infJump=State.infJumpEnabled, antiRagdoll1=State.antiRagdoll1Enabled, antiRagdoll2=State.antiRagdoll2Enabled, fpsBoost=State.fpsBoostEnabled,
                        ...
                        autoTpDown=State.autoTpDownEnabled, autoTpDownY=State.autoTpDownY, autoTpDownDelay=State.autoTpDownDelay,
                        infJumpMode=State.infJumpMode,
                        toolSpammerEnabled=State.toolSpammerEnabled,
                        ...
                        keyLock=State.keyLock.Name, keyToolSpam=State.keyToolSpam.Name,
                        keyAutoGrab=State.keyAutoGrab.Name, keyInfJump=State.keyInfJump.Name,
                        keyAutoTpDownToggle=State.keyAutoTpDownToggle.Name, keyAntiRag1=State.keyAntiRag1.Name,
                        keyAntiRag2=State.keyAntiRag2.Name, keyFps=State.keyFps.Name,
                        keyMedusa=State.keyMedusa.Name, keyUnwalk=State.keyUnwalk.Name,
                        keyCarry=State.keyCarry.Name,
                }
                pcall(function() writefile("VoidHubConfig.json", HttpService:JSONEncode(cfg)) end)
                saveDebounce = false
        end)
end
...
local function createMobilePanel()
        ...
        local dropBRSetActive = makeSeparateBtn("BtnDropBR", "DROP\nBR",
                UDim2.new(1, COL1_X, BASE_Y, ROW_Y[1]), "VoidBtn_dropbr.txt",
        ...
        local autoLeftSetActive = makeSeparateBtn("BtnAutoLeft", "AUTO\nLEFT",
                UDim2.new(1, COL2_X, BASE_Y, ROW_Y[1]), "VoidBtn_autoleft.txt",
                function(setActive)
        ...
        local lockSetActive = makeSeparateBtn("BtnLock", "TARGET\nLOCK",
                UDim2.new(1, COL1_X, BASE_Y, ROW_Y[2]), "VoidBtn_lock.txt",
                function(setActive)
                        State.lockEnabled = not State.lockEnabled
                        if setTargetLock then setTargetLock(State.lockEnabled) end
                        setActive(State.lockEnabled)
                        autoSaveConfig()
                end, true)

        local autoRightSetActive = makeSeparateBtn("BtnAutoRight", "AUTO\nRIGHT",
                UDim2.new(1, COL2_X, BASE_Y, ROW_Y[2]), "VoidBtn_autoright.txt",
                function(setActive)
        ...
        local tpDownSetActive = makeSeparateBtn("BtnTpDown", "TP\nDOWN",
                UDim2.new(1, COL1_X, BASE_Y, ROW_Y[3]), "VoidBtn_tpdown.txt",
                function(setActive, currentActive)
                        tpToGround()
        ...
        local carrySpeedSetActive = makeSeparateBtn("BtnCarrySpd", "CARRY\nSPD",
                UDim2.new(1, COL2_X, BASE_Y, ROW_Y[3]), "VoidBtn_carryspd.txt",
                function(setActive)
                        toggleSpeedType()
                        setActive(State.speedType == "carry")
                end, true)

        local toolSpamSetActive = makeSeparateBtn("BtnToolSpam", "TOOL\nSPAM",
                UDim2.new(1, COL1_X, BASE_Y, ROW_Y[4]), "VoidBtn_toolspam.txt",
                function(setActive)
                        State.toolSpammerEnabled = not State.toolSpammerEnabled
                        if setToolSpammer then setToolSpammer(State.toolSpammerEnabled) end
                        if State.toolSpammerEnabled then startToolSpammer() else stopToolSpammer() end
                        setActive(State.toolSpammerEnabled)
                        autoSaveConfig()
                end, true)
        ...
        task.spawn(function()
                ...
                if MobileButtons.Buttons.lock then MobileButtons.Buttons.lock(State.lockEnabled) end
                if MobileButtons.Buttons.carrySpeed then MobileButtons.Buttons.carrySpeed(State.speedType == "carry") end
                if MobileButtons.Buttons.toolSpammer then MobileButtons.Buttons.toolSpammer(State.toolSpammerEnabled) end
        end)
...
local C_ACCENT  = Color3.fromRGB(130, 40, 255)
local C_ACCENT2 = Color3.fromRGB(160, 100, 255)
local C_DIM     = Color3.fromRGB(120, 120, 120)
local C_WHITE   = Color3.fromRGB(255,255,255)
local C_ON_BG   = Color3.fromRGB(80, 20, 160)
...
for _, name in pairs({"JispiHubGUI","VexulHubGUI","VoidHubGUI"}) do
...
gui.Name="VoidHubGUI"; gui.ResetOnSpawn=false; gui.DisplayOrder=10
...
local function saveStealBarPosition()
    ...
    local str = string.format("%.3f,%.1f,%.3f,%.1f", pos.X.Scale, pos.X.Offset, pos.Y.Scale, pos.Y.Offset)
    pcall(function() writefile("VoidStealBarPos.txt", str) end)
end

local function loadStealBarPosition()
    local savedPos = nil
    pcall(function() savedPos = readfile("VoidStealBarPos.txt") end)
    if savedPos and savedPos ~= "" then
        local parts = {}
        for part in string.gmatch(savedPos, "[^,]+") do table.insert(parts, part) end
        if #parts >= 4 then
...
-- ================= JANELA DE KEYBINDS (TRACKER) =================
local kbWindow = Instance.new("Frame", gui)
kbWindow.Name = "VoidKeybindsWindow"
kbWindow.Size = UDim2.new(0, 180, 0, 220)
kbWindow.Position = UDim2.new(1, -200, 0, 20)
kbWindow.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
kbWindow.BackgroundTransparency = 0.5
kbWindow.BorderSizePixel = 0
kbWindow.ClipsDescendants = true
kbWindow.ZIndex = 8
Instance.new("UICorner", kbWindow).CornerRadius = UDim.new(0, 10)
Instance.new("UIStroke", kbWindow).Color = C_BORDER2

local kbHeader = Instance.new("Frame", kbWindow)
kbHeader.Size = UDim2.new(1, 0, 0, 30)
kbHeader.BackgroundColor3 = Color3.fromRGB(15, 15, 15)
kbHeader.BackgroundTransparency = 0.5
kbHeader.BorderSizePixel = 0
kbHeader.ZIndex = 9
Instance.new("UICorner", kbHeader).CornerRadius = UDim.new(0, 10)
local kbHeaderDiv = Instance.new("Frame", kbHeader)
kbHeaderDiv.Size = UDim2.new(1, 0, 0, 1)
kbHeaderDiv.Position = UDim2.new(0, 0, 1, -1)
kbHeaderDiv.BackgroundColor3 = C_BORDER
kbHeaderDiv.BorderSizePixel = 0
kbHeaderDiv.ZIndex = 10
...
kbTitle.TextColor3 = C_WHITE
kbTitle.Font = Enum.Font.GothamBold
kbTitle.TextSize = 12
kbTitle.TextXAlignment = Enum.TextXAlignment.Left
kbTitle.ZIndex = 10

local kbMinBtn = Instance.new("TextButton", kbHeader)
kbMinBtn.Size = UDim2.new(0, 30, 1, 0)
kbMinBtn.Position = UDim2.new(1, -30, 0, 0)
kbMinBtn.BackgroundTransparency = 1
kbMinBtn.Text = "-"
kbMinBtn.TextColor3 = C_WHITE
kbMinBtn.Font = Enum.Font.GothamBlack
kbMinBtn.TextSize = 16
kbMinBtn.ZIndex = 10

local kbListContainer = Instance.new("ScrollingFrame", kbWindow)
kbListContainer.Size = UDim2.new(1, 0, 1, -35)
kbListContainer.Position = UDim2.new(0, 0, 0, 35)
kbListContainer.BackgroundTransparency = 1
kbListContainer.BorderSizePixel = 0
kbListContainer.ScrollBarThickness = 2
kbListContainer.ScrollBarImageColor3 = C_BORDER2
kbListContainer.AutomaticCanvasSize = Enum.AutomaticSize.Y
kbListContainer.CanvasSize = UDim2.new(0, 0, 0, 0)
kbListContainer.ZIndex = 9

local kbListLayout = Instance.new("UIListLayout", kbListContainer)
kbListLayout.SortOrder = Enum.SortOrder.LayoutOrder
kbListLayout.Padding = UDim.new(0, 4)

local kbPad = Instance.new("UIPadding", kbListContainer)
kbPad.PaddingLeft = UDim.new(0, 10)
kbPad.PaddingRight = UDim.new(0, 10)
kbPad.PaddingTop = UDim.new(0, 5)

-- Logic minimize/maximize
local kbMinimized = false
kbMinBtn.MouseButton1Click:Connect(function()
    kbMinimized = not kbMinimized
    if kbMinimized then
        kbMinBtn.Text = "+"
        kbWindow.Size = UDim2.new(0, 180, 0, 30)
    else
        kbMinBtn.Text = "-"
        kbWindow.Size = UDim2.new(0, 180, 0, 220)
    end
...
-- Draggable Keybind Window
makeMainDraggable(kbWindow)

local function saveKbWindowPos()
    local pos = kbWindow.Position
    local str = string.format("%.3f,%.1f,%.3f,%.1f", pos.X.Scale, pos.X.Offset, pos.Y.Scale, pos.Y.Offset)
    pcall(function() writefile("VoidKeybindsWindowPos.txt", str) end)
end

local function loadKbWindowPos()
    local savedPos = nil
    pcall(function() savedPos = readfile("VoidKeybindsWindowPos.txt") end)
    if savedPos and savedPos ~= "" then
        local parts = {}
        for part in string.gmatch(savedPos, "[^,]+") do table.insert(parts, part) end
        if #parts >= 4 then
            kbWindow.Position = UDim2.new(tonumber(parts[1]), tonumber(parts[2]), tonumber(parts[3]), tonumber(parts[4]))
        end
    end
end

local kbLastSave = 0
kbWindow:GetPropertyChangedSignal("Position"):Connect(function()
    if tick() - kbLastSave > 0.5 then
        kbLastSave = tick()
        saveKbWindowPos()
    end
end)

local MasterKeybindsList = {
    {Name = "Target Lock", Key = "keyLock"},
    {Name = "Tool Spammer", Key = "keyToolSpam"},
    {Name = "Auto Grab", Key = "keyAutoGrab"},
    {Name = "Infinite Jump", Key = "keyInfJump"},
    {Name = "Auto Tp Down", Key = "keyAutoTpDownToggle"},
    {Name = "Anti Ragdoll 1", Key = "keyAntiRag1"},
    {Name = "Anti Ragdoll 2", Key = "keyAntiRag2"},
    {Name = "FPS Boost", Key = "keyFps"},
    {Name = "Medusa Counter", Key = "keyMedusa"},
    {Name = "Unwalk", Key = "keyUnwalk"},
    {Name = "Drop Brainrot", Key = "keyDropBR"},
    {Name = "Tp Down", Key = "keyTpDown"},
    {Name = "Auto Left", Key = "keyAutoLeft"},
    {Name = "Auto Right", Key = "keyAutoRight"},
    {Name = "Carry Mode", Key = "keyCarry"},
}

UpdateKeybindUI = function()
    ...
    local count = 0
    for i, item in ipairs(MasterKeybindsList) do
        local currentKey = State[item.Key]
        if currentKey and currentKey ~= Enum.KeyCode.Unknown then
            count = count + 1
            local row = Instance.new("TextLabel", kbListContainer)
            row.Size = UDim2.new(1, 0, 0, 16)
            row.BackgroundTransparency = 1
            row.Text = string.format("[%s] %s", currentKey.Name, item.Name)
    ...
    if count == 0 then
        local empty = Instance.new("TextLabel", kbListContainer)
        empty.Size = UDim2.new(1, 0, 0, 16)
        empty.BackgroundTransparency = 1
        empty.Text = "Nenhuma Keybind"
...
local function saveMainPosition()
        ...
        local str = string.format("%.3f,%.1f,%.3f,%.1f", pos.X.Scale, pos.X.Offset, pos.Y.Scale, pos.Y.Offset)
        pcall(function() writefile("VoidHubGUIPos.txt", str) end)
end
...
local function saveMiniPosition()
        if miniBtn then
                ...
                local str = string.format("%.3f,%.1f,%.3f,%.1f", pos.X.Scale, pos.X.Offset, pos.Y.Scale, pos.Y.Offset)
                pcall(function() writefile("VoidMiniPos.txt", str) end)
        end
end

local function loadMainPosition()
        local savedPos = nil
        pcall(function() savedPos = readfile("VoidHubGUIPos.txt") end)
        if savedPos and savedPos ~= "" then
                local parts = {}
                for part in string.gmatch(savedPos, "[^,]+") do table.insert(parts, part) end
                if #parts >= 4 then
...
local function loadMiniPosition()
        local savedPos = nil
        pcall(function() savedPos = readfile("VoidMiniPos.txt") end)
...
titleLbl.BackgroundTransparency=1; titleLbl.Text="VOID HUB"
...
local function makeInputRow(label, default, onChange)
        ...
        box.FocusLost:Connect(function(enterPressed)
                ...
                if num ~= nil then
                        local finalVal = math.clamp(num, 0, 500)
        ...
        if keybindStateKey then
                ...
                local function nkStop(key)
                        ...
                        if key then
                                ...
                                autoSaveConfig()
                                if UpdateKeybindUI then UpdateKeybindUI() end
                        end
                end
...
setCarryToggle = makeToggleRow("Carry Mode", nil, State.speedType == "carry", function(on)
        if on then State.speedType = "carry" else State.speedType = "normal" end
        autoSaveConfig()
        if MobileButtons.Buttons.carrySpeed then MobileButtons.Buttons.carrySpeed(on) end
end, nil, "keyCarry")
makeGap(10)

makeSectionLabel("Combat")
setTargetLock = makeToggleRow("Target Lock", nil, false,
        function(on) State.lockEnabled=on; if MobileButtons.Buttons.lock then MobileButtons.Buttons.lock(on) end end,
        nil, "keyLock")

-- TOOL SPAMMER
local toolSpammerConn = nil
local function startToolSpammer()
    if toolSpammerConn then return end
    toolSpammerConn = RunService.Heartbeat:Connect(function()
        if State.toolSpammerEnabled then
            local char = LP.Character
            if char then
...
local function stopToolSpammer()
    if toolSpammerConn then toolSpammerConn:Disconnect(); toolSpammerConn = nil end
end

setToolSpammer = makeToggleRow("Tool Spammer", nil, false, function(on)
        State.toolSpammerEnabled = on
        if on then startToolSpammer() else stopToolSpammer() end
        if MobileButtons.Buttons.toolSpammer then MobileButtons.Buttons.toolSpammer(on) end
        autoSaveConfig()
end, nil, "keyToolSpam")
makeGap(10)
...
setInfJump = makeToggleRow("Infinite Jump", nil, false, function(on) State.infJumpEnabled=on end, nil, "keyInfJump")
...
local function ar2IsRagdolled()
    if not AR2.charData.humanoid then return false end
    local state = AR2.charData.humanoid:GetState()
...
local function ar2ForceExit()
    if not AR2.charData.humanoid or not AR2.charData.root then return end
    pcall(function() LP:SetAttribute("RagdollEndTime", workspace:GetServerTimeNow()) end)
    for _, d in ipairs(AR2.charData.character:GetDescendants()) do
        if d:IsA("BallSocketConstraint") or (d:IsA("Attachment") and d.Name:find("RagdollAttachment")) then d:Destroy() end
    end
    if not AR2.isBoosting then AR2.isBoosting = true; AR2.charData.humanoid.WalkSpeed = AR2.BOOST_SPEED end
    if AR2.charData.humanoid.Health > 0 then AR2.charData.humanoid:ChangeState(Enum.HumanoidStateType.Running) end
    AR2.charData.root.Anchored = false
end
local function ar2Loop()
    while AR2.mode == "on" do
        task.wait()
        if ar2IsRagdolled() then ar2ForceExit()
        elseif AR2.isBoosting then
            AR2.isBoosting = false
            if AR2.charData.humanoid then AR2.charData.humanoid.WalkSpeed = AR2.DEFAULT_SPEED end
        end
    end
end
local function enableAR2()
    if AR2.mode == "on" then return end
    if not ar2Cache() then return end
    AR2.mode = "on"
    table.insert(AR2.connections, RunService.RenderStepped:Connect(function() local cam = workspace.CurrentCamera; if cam and AR2.charData.humanoid then cam.CameraSubject = AR2.charData.humanoid end end))
    table.insert(AR2.connections, LP.CharacterAdded:Connect(function() AR2.isBoosting = false; task.wait(0.5); ar2Cache() end))
    task.spawn(ar2Loop)
end
local function disableAR2()
    AR2.mode = nil
    if AR2.isBoosting and AR2.charData.humanoid then AR2.charData.humanoid.WalkSpeed = AR2.DEFAULT_SPEED end
    AR2.isBoosting = false
    ar2DisconnectAll()
    AR2.charData = {}
end

setAntiRag1 = makeToggleRow("Anti Ragdoll 1", nil, false, function(on)
        State.antiRagdoll1Enabled=on; if on then startAntiRagdoll1() else stopAntiRagdoll1() end
end, nil, "keyAntiRag1")

setAntiRag2 = makeToggleRow("Anti Ragdoll 2", nil, false, function(on)
        State.antiRagdoll2Enabled=on; if on then enableAR2() else disableAR2() end
end, nil, "keyAntiRag2")
...
setMenuDropBR = makeToggleRow("Drop Brainrot", nil, false, function(on)
        if on then
                ...
                task.delay(0.5, function()
                        if setMenuDropBR then setMenuDropBR(false) end
                        if MobileButtons.Buttons and MobileButtons.Buttons.dropBR then MobileButtons.Buttons.dropBR(false) end
                end)
        end
end, nil, "keyDropBR")
...
setMenuTpDown = makeToggleRow("Tp Down", nil, false, function(on)
        if on then
                ...
                task.delay(0.5, function()
                        if setMenuTpDown then setMenuTpDown(false) end
                        if MobileButtons.Buttons and MobileButtons.Buttons.tpDown then MobileButtons.Buttons.tpDown(false) end
                end)
        end
end, nil, "keyTpDown")
...
footerLbl.Text="void hub  ·  v1.0"; footerLbl.TextColor3=Color3.fromRGB(50,50,50)
...
UIS.InputBegan:Connect(function(inp, gp)
        ...
        if State.keyLock ~= Enum.KeyCode.Unknown and kc == State.keyLock then
            State.lockEnabled = not State.lockEnabled
            if setTargetLock then setTargetLock(State.lockEnabled) end
            if MobileButtons.Buttons.lock then MobileButtons.Buttons.lock(State.lockEnabled) end
            autoSaveConfig()
        ...
        -- New Keybinds mappings:
        if State.keyToolSpam ~= Enum.KeyCode.Unknown and kc == State.keyToolSpam then
            State.toolSpammerEnabled = not State.toolSpammerEnabled
            if setToolSpammer then setToolSpammer(State.toolSpammerEnabled) end
            if State.toolSpammerEnabled then startToolSpammer() else stopToolSpammer() end
            if MobileButtons.Buttons.toolSpammer then MobileButtons.Buttons.toolSpammer(State.toolSpammerEnabled) end
        ...
        if State.keyAutoGrab ~= Enum.KeyCode.Unknown and kc == State.keyAutoGrab then
            AutoSteal.Enabled = not AutoSteal.Enabled
            if setInstaGrab then setInstaGrab(AutoSteal.Enabled) end
            if AutoSteal.Enabled then startAutoSteal() else stopAutoSteal() end
            autoSaveConfig()
        end
        if State.keyInfJump ~= Enum.KeyCode.Unknown and kc == State.keyInfJump then
            State.infJumpEnabled = not State.infJumpEnabled
            if setInfJump then setInfJump(State.infJumpEnabled) end
            autoSaveConfig()
        end
        if State.keyAutoTpDownToggle ~= Enum.KeyCode.Unknown and kc == State.keyAutoTpDownToggle then
            State.autoTpDownEnabled = not State.autoTpDownEnabled
            if setAutoTpDown then setAutoTpDown(State.autoTpDownEnabled) end
            autoSaveConfig()
        end
        if State.keyAntiRag1 ~= Enum.KeyCode.Unknown and kc == State.keyAntiRag1 then
            State.antiRagdoll1Enabled = not State.antiRagdoll1Enabled
            if setAntiRag1 then setAntiRag1(State.antiRagdoll1Enabled) end
            if State.antiRagdoll1Enabled then startAntiRagdoll1() else stopAntiRagdoll1() end
            autoSaveConfig()
        end
        if State.keyAntiRag2 ~= Enum.KeyCode.Unknown and kc == State.keyAntiRag2 then
            State.antiRagdoll2Enabled = not State.antiRagdoll2Enabled
            if setAntiRag2 then setAntiRag2(State.antiRagdoll2Enabled) end
            if State.antiRagdoll2Enabled then enableAR2() else disableAR2() end
            autoSaveConfig()
        end
        if State.keyFps ~= Enum.KeyCode.Unknown and kc == State.keyFps then
        ...
        if State.keyMedusa ~= Enum.KeyCode.Unknown and kc == State.keyMedusa then
            State.medusaCounterEnabled = not State.medusaCounterEnabled
        ...
        if State.keyUnwalk ~= Enum.KeyCode.Unknown and kc == State.keyUnwalk then
        ...
        if State.keyCarry ~= Enum.KeyCode.Unknown and kc == State.keyCarry then
            if State.speedType == "carry" then State.speedType = "normal" else State.speedType = "carry" end
            if setCarryToggle then setCarryToggle(State.speedType == "carry") end
            if MobileButtons.Buttons.carrySpeed then MobileButtons.Buttons.carrySpeed(State.speedType == "carry") end
            autoSaveConfig()
        end
...
local function loadConfig()
        local hasFile=false; pcall(function() hasFile=isfile("VoidHubConfig.json") end)
        if not hasFile then return end
        local ok,cfg=pcall(function() return HttpService:JSONDecode(readfile("VoidHubConfig.json")) end)
        ...
        if cfg.autoTpDownDelay and type(cfg.autoTpDownDelay)=="number" then State.autoTpDownDelay=cfg.autoTpDownDelay end
        if cfg.antiRagdoll1 then State.antiRagdoll1Enabled=true;   setAntiRag1(true); startAntiRagdoll1() end
        if cfg.antiRagdoll2 then State.antiRagdoll2Enabled=true;   setAntiRag2(true); enableAR2() end
        ...
        if type(cfg.lockEnabled) == "boolean" then State.lockEnabled = cfg.lockEnabled end
        if type(cfg.toolSpammerEnabled) == "boolean" then State.toolSpammerEnabled = cfg.toolSpammerEnabled; if cfg.toolSpammerEnabled then startToolSpammer() end end
        ...
        loadKey("keyLock",      "keyLock")
        loadKey("keyToolSpam",  "keyToolSpam")
        loadKey("keyAutoGrab",  "keyAutoGrab")
        loadKey("keyInfJump",   "keyInfJump")
        loadKey("keyAutoTpDownToggle", "keyAutoTpDownToggle")
        loadKey("keyAntiRag1",  "keyAntiRag1")
        loadKey("keyAntiRag2",  "keyAntiRag2")
        loadKey("keyFps",       "keyFps")
        loadKey("keyMedusa",    "keyMedusa")
        loadKey("keyUnwalk",    "keyUnwalk")
        loadKey("keyCarry",     "keyCarry")
...
local function setupChar(char)
        ...
        if State.antiRagdoll1Enabled and not Conns.antiRag then task.wait(0.5); startAntiRagdoll1() end
        if State.antiRagdoll2Enabled then enableAR2() end
...
RunService.Heartbeat:Connect(function()
        ...
        if State.autoTpDownEnabled then
                ...
                if curY >= State.autoTpDownY then
                        if tick() - autoTpDownLastTick >= State.autoTpDownDelay then
                                autoTpDownLastTick = tick()
                                tpToGround()
                        end
                end
        end
...
RunService.RenderStepped:Connect(function()
        ...
        elseif (State.antiRagdoll1Enabled or State.antiRagdoll2Enabled) and State.lastMoveDir.Magnitude > 0 then
                local anyHeld = false
...
RunService.Heartbeat:Connect(function()
        if not (State.lockEnabled and h and hrp) then return end
